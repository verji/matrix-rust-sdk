...
//!
//! Dehydrated devices intent to solve the use-case where users might want to
...
//! the homeserver. A dehydrated device will act as a normal device from the
//! point of view of other devices. It will upload device and one-time keys to
...
//! device and one-time keys will be encrypted and uploaded to the homeserver as
//! well.
//!
...
use crate::{
    ...
    OlmError, OlmMachine, ReadOnlyAccount, SignatureError,
};

/// Error type for device dehydration issues.
#[derive(Debug, Error)]
pub enum DehydrationError {
    /// The dehydrated device could not have been unpickled.
    #[error(transparent)]
    Pickle(#[from] PickleError),
    /// The dehydrated device could not have been signed by our user identity,
...
impl DehydratedDevices {
    ...
    /// Rehydrated an existing dehydrated device.
    ///
...
impl RehydratedDevice {
    ...
    ///     imported_room_keys += rehydrated.receive_events(response.events).await?;
    /// }
    ///
