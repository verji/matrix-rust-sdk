...
async fn test_new_pinned_events_are_added_on_sync() {
    ...
    // Join the room
    let _ = test_helper.setup_sync_response(Vec::new(), None).await;
    ...
    let timeline =
        Timeline::builder(&room).with_focus(TimelineFocus::PinnedEvents).build().await.unwrap();
    test_helper.server.reset().await;
    ...
    // Load new pinned event contents from sync
    let _ = test_helper
        .setup_sync_response(
    ...
    let next_diff = timeline_stream.next().await.unwrap();
    if let VectorDiff::PushBack { value } = next_diff {
        assert_eq!(value.as_event().unwrap().event_id().unwrap(), event_id!("$2"));
    } else {
        panic!("Expected the vector diff to be a PushBack one");
    }
    assert_pending!(timeline_stream);
    test_helper.server.reset().await;
}
...
async fn test_new_pinned_event_ids_reload_the_timeline() {
    ...
    assert!(matches!(timeline_stream.next().await.unwrap(), VectorDiff::Clear));
    if let VectorDiff::PushBack { value } =
        timeline_stream.next().await.expect("Expected the vector diff to be a PushBack one")
    {
    ...
    if let VectorDiff::PushFront { value } =
        timeline_stream.next().await.expect("Expected the vector diff to be a PushFront one")
    {
