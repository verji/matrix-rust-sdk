...
use matrix_sdk_test::{DEFAULT_TEST_ROOM_ID, EventBuilder, JoinedRoomBuilder, StateTestEvent, SyncResponseBuilder, test_json};
use ruma::{api::client::membership::get_member_events, device_id, events::room::member::{RoomMemberEvent, RoomMemberEventContent}, owned_room_id, serde::Raw, user_id, OwnedUserId, owned_user_id, MilliSecondsSinceUnixEpoch, EventId, UInt, OwnedEventId};
use ruma::events::room::pinned_events::RoomPinnedEventsEventContent;
use serde::Serialize;
use serde_json::json;
use tokio::runtime;
...
pub fn load_pinned_events_benchmark(c: &mut Criterion) {
    ...
    // for i in 0 .. PINNED_EVENTS_COUNT {
    //     let ev_id = EventId::parse(format!("${i}")).expect("Invalid event id");
    //     let ts = MilliSecondsSinceUnixEpoch(UInt::new(i as u64).expect("Invalid TS"));
    //     let ev = f.text_msg(format!("Message {i}")).event_id(&ev_id).server_ts(ts).into_raw_sync();
    //     joined_room_builder = joined_room_builder.add_timeline_event(ev);
    // }
    let pinned_event_ids: Vec<OwnedEventId> = (0 .. PINNED_EVENTS_COUNT).map(|i| {
        EventId::parse(format!("${i}")).expect("Invalid event id")
    }).collect();
    joined_room_builder = joined_room_builder.add_state_event(StateTestEvent::Custom(json!(
        {
            "content": {
                "pinned": pinned_event_ids.clone()
            },
    ...
    let response_json = sync_response_builder.add_joined_room(joined_room_builder).build_json_sync_response();
    ...
    runtime.block_on(
        Mock::given(method("GET"))
            ...
            .respond_with(move |r: &Request| {
                ...
                let event = f.text_msg(format!("Message {event_id_str}")).event_id(&event_id).server_ts(MilliSecondsSinceUnixEpoch::now()).into_raw_sync();
                ResponseTemplate::new(200).set_delay(Duration::from_millis(50)).set_body_json(event.json())
            })
            .mount(&server)
    );
    // runtime.block_on(server.reset());
    ...
    group.bench_function(BenchmarkId::new("load_pinned_events", name), |b| {
        b.to_async(&runtime).iter(|| async {
            ...
            let timeline = Timeline::builder(&room).with_focus(TimelineFocus::PinnedEvents).build().await.expect("Could not create timeline");
