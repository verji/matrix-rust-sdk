...
async fn test_redacting_reaction() -> Result<()> {
    ...
    timeline.send(RoomMessageEventContent::text_plain("hi!").into(), None).await;
    {
        ...
        assert_eq!(event.is_local_echo(), true);
        assert_eq!(event.reactions().is_empty(), true);
    }
    ...
    // Check we have the remote echo
    {
        ...
        assert_eq!(event.is_local_echo(), false);
    };
    ...
    // Send a reaction
    {
        let reaction: ReactionEventContent =
            Annotation::new(event_id.clone(), reaction_key.into()).into();
        timeline.send(reaction.into(), None).await;
    }
    ...
    let (reaction_tx_id, reaction_event_id) = {
        ...
        let reaction = reactions.user_reaction(user_id).unwrap();
        reaction.to_owned()
    };
    ...
    let reaction_event_id: OwnedEventId = {
        let event = assert_event_is_updated(&mut stream, &event_id, message_position).await;
        let reactions = event.reactions().get(reaction_key).unwrap();
    ...
    // Redact the reaction
    timeline.redact(&reaction_event_id, None /* TODO? */, None).await;
